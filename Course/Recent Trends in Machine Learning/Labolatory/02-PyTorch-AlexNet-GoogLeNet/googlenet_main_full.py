...
from google_net_full import GoogLeNetFull
...
#create googlenet instance
GoogLeNetFull = GoogLeNetFull()
GoogLeNetFull = GoogLeNetFull.to(device)

# CrossEntropyLoss for multinomial classification (because we have 10 classes)
criterion = nn.CrossEntropyLoss()
params_to_update = GoogLeNetFull.parameters()
optimizer = optim.SGD(params_to_update, lr=0.001, momentum=0.9)

#Train!
best_model, val_acc_history, loss_acc_history = train_model(GoogLeNetFull, dataloaders, criterion, optimizer, device, 10, 'googlenetFull_module_lr_0.001_bestsofar',is_inception=True)

#Save training statistics 
with open('googlenetFull_module_lr_0.001_bestsofar.pkl','wb') as f:
...
GoogLeNetFull.load_state_dict(torch.load('googlenetFull_module_lr_0.001_bestsofar.pth'))
test_dataloaders = { 'test': test_dataloader }
test_acc, test_loss = test_model(GoogLeNetFull, test_dataloaders, criterion, device)
