...
from alexnet_module_LRN import AlexNetModule_LRN
...
AlexNetModule_LRN = AlexNetModule_LRN(10).to(device)

# CrossEntropyLoss for multinomial classification (because we have 10 classes)
criterion = nn.CrossEntropyLoss()
params_to_update = AlexNetModule_LRN.parameters()
optimizer = optim.SGD(params_to_update, lr=0.001, momentum=0.9)

dataloaders = { 'train': train_dataloader, 'val': val_dataloader }

#Train!
best_model, val_acc_history, loss_acc_history = train_model(AlexNetModule_LRN, dataloaders, criterion, optimizer, device, 10, 'alex_module_LRN_lr_0.001_bestsofar')

#Save training statistics 
with open('alex_module_LRN_lr_0.001_bestsofar.pkl','wb') as f:
...
AlexNetModule_LRN.load_state_dict(torch.load('alex_module_LRN_lr_0.001_bestsofar.pth'))
test_dataloaders = { 'test': test_dataloader }
test_acc, test_loss = test_model(AlexNetModule_LRN, test_dataloaders, criterion, device)
