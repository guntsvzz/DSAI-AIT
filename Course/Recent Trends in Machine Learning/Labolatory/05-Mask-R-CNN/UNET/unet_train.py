...
parser.add_argument("--datadir", type=str, default="/root/Datasets/Cityscapes/", help="directory the Cityscapes dataset is in")
...
img_batch = DataLoader(img_data, batch_size=args.batch_size, shuffle=True, num_workers=0)
print(img_data)
...
generator = nn.DataParallel(UnetGenerator(3, img_data.num_classes, 64), device_ids=[i for i in range(args.num_gpu)]).to(device)
...
for epoch in range(epochs):
    for idx_batch, (imagergb, labelmask, labelrgb) in enumerate(img_batch):
        ...
        if idx_batch % 1 == 0:
            ...
            v_utils.save_image(x.to(device).data, "./result/original_image_{}_{}.png".format(epoch, idx_batch))
            ...
            if args.losstype == "segment":
                ...
                v_utils.save_image(y.data, "./result/gen_image_{}_{}.png".format(epoch, idx_batch))
